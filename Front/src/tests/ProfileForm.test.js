  ...
  useForm: () => ({
    ...
    formState: {
      errors: {
        ...
        livre: {
          message: "Le titre du livre doit contenir au moins 2 caractères.",
        },
...
jest.mock("@/app/ui/alert-dialog", () => ({
  ...
  AlertDialogDescription: function AlertDialogDescription({
    children,
    className,
  }) {
    return <div className={className}>{children}</div>;
...
describe("ProfileForm", () => {
  ...
  it("renders form elements", () => {
    ...
    expect(
      screen.getByRole("textbox", { name: /Auteur\/ice/i })
    ).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: /Livre/i })).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: /Phrase/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Ajouter/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Arrière/i })
    ).toBeInTheDocument();
  });
  ...
  it("displays correct placeholders for all fields", () => {
    ...
    expect(
      screen.getByPlaceholderText("Écrire le nom de l'auteur")
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Écrire le titre du livre")
    ).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Écrire ta citation")
    ).toBeInTheDocument();
  });
  ...
  it("validates required fields", async () => {
    ...
    expect(
      await screen.findByText("L'auteur doit contenir au moins 2 caractères.")
    ).toBeInTheDocument();
    expect(
      await screen.findByText(
        "Le titre du livre doit contenir au moins 2 caractères."
      )
    ).toBeInTheDocument();
    expect(
      await screen.findByText(
        "La citation doit contenir au moins 5 caractères."
      )
    ).toBeInTheDocument();
  });
  ...
  it("shows validation errors when fields have insufficient length", async () => {
    ...
    expect(
      screen.getByText("L'auteur doit contenir au moins 2 caractères.")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Le titre du livre doit contenir au moins 2 caractères.")
    ).toBeInTheDocument();
    expect(
      screen.getByText("La citation doit contenir au moins 5 caractères.")
    ).toBeInTheDocument();
  });
  ...
  it("handles successful form submission", async () => {
    ...
    expect(
      await screen.findByText("Bravo! Citation ajoutée correctement!")
    ).toBeInTheDocument();
  });
  ...
  it("handles failed form submission", async () => {
    ...
    expect(
      await screen.findByText("Ooops! Erreur dans l'ajout de la citation")
    ).toBeInTheDocument();
  });
  ...
  it('navigates to "/phrases" when dialog is closed', async () => {
    ...
    const errorMessage = await screen.findByText(
      "Ooops! Erreur dans l'ajout de la citation"
    );
